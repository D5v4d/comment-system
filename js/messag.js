...
class Messag {
    ...
    initMesseage() {
        this.filterFlag = JSON.parse(localStorage.getItem("filterFlag"));
        // localStorage.clear()
        ...
        let sentNumber = document.querySelectorAll(".block__comment__sent__number");
        ...
        let imgFavorites = document.querySelectorAll(".img__favorites");
        if (imgFavorites)
            imgFavorites.forEach((element) => {
                let nameSent = element.parentNode.querySelector(".block__comment__full__name").innerHTML;
                let textSent = element.parentNode.querySelector(".block__comment__text").innerHTML;
                this.listComments.forEach((comment) => {
                    if (comment.favorites) {
                        if (comment.text == textSent &&
                    ...
                    this.listComments.forEach((comment) => {
                        if (comment.text == textSent &&
                            ...
                            comment.answer.forEach((comment, index) => {
                                if (index == commentAnswer.length - 1)
                                    this.renderMessage(comment);
                            });
                        }
                    });
    ...
    renderMessage(comment) {
        ...
        let commentData = `${data.getDate()}.${data.getMonth()} ${data.getHours()}:${(data.getMinutes() < 10 ? "0" : "") + data.getMinutes()}`;
    ...
    ratingMessages() {
        document.addEventListener("click", (e) => {
            ...
            if (target.classList.contains("block__comment__sent__minus") ||
                ...
                this.listComments = JSON.parse(localStorage.getItem("comments")) ?? [];
                let rating = target.parentNode.querySelector(".block__comment__sent__number");
    ...
    favoriteMessages() {
        this.headFavorites.addEventListener(`click`, () => {
            ...
            let commentAnswer = document.querySelector(".block__comment__answer");
            let sentAnswer = document.querySelector(".block__comment__sent__answer");
            let imgFavorites = document.querySelector(".img__favorites");
        ...
        document.addEventListener("click", (e) => {
            ...
            if (target.classList.contains("block__comment__sent__favorites") ||
                ...
                this.listComments.forEach((comment) => {
                    ...
                    if (comment.text == textSent &&
                        `${comment.name} ${comment.surname}` == nameSent) {
                        let imgFavorites = target.parentNode.querySelector(".img__favorites");
                        if (faforitesFlag) {
